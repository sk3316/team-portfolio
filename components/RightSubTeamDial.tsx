...
export default function RightSubTeamDial({
  ...
  const angleStep = 30; // Degrees between sub-team pills
  const radius = 260; // Radius of circular arc
  const originXFromRight = -130; // Virtual origin off-screen to the right
  ...
  return (
    <div
      onWheel={handleWheel}
      className="absolute right-0 top-0 bottom-0 w-64 select-none z-20 flex items-center justify-end pointer-events-auto"
      style={{ touchAction: "none" }}
    >
      {/* Faint Orbit Guideline (Mirrored Arc) */}
      <svg
        className="absolute right-0 top-1/2 -translate-y-1/2 w-72 h-[600px] pointer-events-none opacity-20"
      ...
      <div className="absolute right-[130px] top-1/2 -translate-y-1/2 flex items-center gap-1.5 pointer-events-none z-30 flex-row-reverse">
        <div className="w-2.5 h-2.5 rounded-full bg-indigo-400 shadow-[0_0_14px_rgba(129,140,248,1)] animate-pulse" />
        <div className="w-6 h-[1.5px] bg-gradient-to-l from-indigo-500/80 to-transparent" />
      </div>
      ...
      <AnimatePresence>
        {!hasScrolled && subTeams.length > 1 && (
          <motion.div
            ...
            className="absolute right-5 bottom-8 z-40 flex flex-col items-center gap-1 pointer-events-none"
          >
      ...
      <div className="relative w-full h-full">
        {subTeams.map((team, index) => {
          ...
          const scale = isSelected ? 1.05 : Math.max(0.75, 1 - distance * 0.1);
          ...
          return (
            <motion.div
              ...
              className="absolute right-0 top-1/2 -translate-y-1/2 cursor-pointer group flex items-center flex-row-reverse gap-2"
              style={{ zIndex: isSelected ? 20 : 10 - distance }}
            >
              {/* Sub-Team Badge / Pill */}
              <div
                className={`px-3 py-1.5 rounded-2xl flex items-center gap-1.5 transition-all duration-300 ${
                  isSelected
              ...
              >
                ...
                />
                <span className="text-xs tracking-wide whitespace-nowrap font-medium max-w-[130px] truncate">
                  {team.name}
                </span>

                {/* Live Member count badge */}
                <span
                  className={`ml-0.5 text-[9.5px] font-bold px-1.5 py-0.5 rounded-md transition-all duration-200 ${
                    isSelected
              ...
              {/* Shortcode Badge */}
              <div
                className={`pointer-events-none whitespace-nowrap bg-neutral-900/80 border border-neutral-800 px-1.5 py-0.5 rounded-md text-[9px] font-mono font-bold transition-opacity duration-200 ${
